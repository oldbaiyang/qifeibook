...
# 从douban_top250.json中获取封面URL
with open("douban_top250.json", "r", encoding="utf-8") as f:
    douban_books = json.load(f)

huozhe = [b for b in douban_books if b['title'] == '活着'][0]
douban_cover_url = huozhe['cover']

print(f"豆瓣封面URL: {douban_cover_url}")
...
temp_image = "temp_huozhe.jpg"
resp = requests.get(douban_cover_url, headers=headers)
...
# 上传到PicGo
print("\n上传到图床...")
payload = {"list": [str(Path(temp_image).absolute())]}
